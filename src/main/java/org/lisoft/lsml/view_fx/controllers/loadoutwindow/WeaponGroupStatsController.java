...
public class WeaponGroupStatsController extends AbstractFXController {
     ...
     *
     * @param aGroupMetrics  The group metrics to get data for the current weapon group from.
     * @param aGlobalMetrics The global metrics to get data for the {@link Loadout} from.
     */
    @Inject
    public WeaponGroupStatsController(GroupMetrics aGroupMetrics, LoadoutMetrics aGlobalMetrics) {
        ...
        alphaDamage.textProperty().bind(format("Alpha: %.1h @ %.0h m", aGroupMetrics.alphaDamage,
                                               aGroupMetrics.alphaDamage.displayRange()));
        ...
        final ObjectBinding<Paint> colorBinding = when(aGroupMetrics.alphaGhostHeat.lessThanOrEqualTo(0.0)).then(
                defaultFill).otherwise(Color.RED);
