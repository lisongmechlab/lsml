...
/**
 * This is an abstract base class for Controllers that are at the root of the stage and have custom
 * window decorations.
 *
 * @author Li Song
 */
public abstract class AbstractFXStageController extends AbstractFXController
    implements MessageReceiver {
  /**
   * How many pixels high the grab area for moving the window at the top is if the titleBar region
   * is not specified.
   */
  ...
  /**
   * How many pixels thick the edge around the window border where you can click and resize the
   * window is.
   */
        ...
        (aObs, aOld, aNew) -> {
          ...
          if (aNew) {
            final ObservableList<Screen> screensForRectangle =
                Screen.getScreensForRectangle(mousePrevMouseAbsX, mousePrevMouseAbsY, 1, 1);
  ...
  public void onMouseDragged(MouseEvent e) {
    ...
    if (currentCursor == Cursor.N_RESIZE
    ...
    if (currentCursor == Cursor.S_RESIZE
    ...
    if (currentCursor == Cursor.E_RESIZE
        || currentCursor == Cursor.NE_RESIZE
        || currentCursor == Cursor.SE_RESIZE) {
  ...
  protected abstract void onShow(LSMLStage aStage);
