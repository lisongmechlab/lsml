...
public class ChassisPageController extends AbstractFXController {
  ...
  @FXML private RadioButton factionFilterAll;
  ...
  @FXML private ListView<ChassisGroup> payloadChassis;
  @FXML private CheckBox payloadEndoSteel;
  @FXML private CheckBox payloadFerroFibrous;
  @FXML private LineChart<Double, Double> payloadGraph;
  @FXML private CheckBox payloadMaxArmour;
  @FXML private CheckBox payloadXLEngine;
  @FXML private TableView<DisplayLoadout> tableAssaults;
  @FXML private TableView<DisplayLoadout> tableHeavies;
  @FXML private TableView<DisplayLoadout> tableLights;
  @FXML private TableView<DisplayLoadout> tableMediums;

  @Inject
  public ChassisPageController(
      Settings aSettings,
  ...
  private void setupChassisTable(
    ...
    addAttributeColumn(
        aTable, "Mass", "loadout.chassis.massMax", "The maximal mass of the chassis.");
    addAttributeColumn(
        aTable, "Fctn", "loadout.chassis.faction.uiShortName", "The faction of the chassis.");
    ...
    addAttributeColumn(
        aTable, "JJ", "loadout.jumpJetsMax", "The maximal number of jump jets for this chassis.");

    final TableColumn<DisplayLoadout, Collection<Modifier>> quirksCol =
        new TableColumn<>("Weapon Quirks");
    quirksCol.setCellValueFactory(
  ...
  private void setupPayloadGraph() {
    ...
    for (final Chassis chassis : ChassisDB.lookupAll()) {
      ...
      final ChassisGroup group =
          groups.computeIfAbsent(mass, x -> new ChassisGroup(mass + " tons"));
      group.add(chassis);
    }
  ...
  private void setupTableData(
    ...
    final FilteredList<DisplayLoadout> filtered =
        new FilteredList<>(
            loadouts, new ChassisFilter(aFactionFilter.get(), showMechVariants.getValue()));
    aTable.setItems(filtered);

    showMechVariants.addListener(
        (aObs, aOld, aNew) -> {
          filtered.setPredicate(new ChassisFilter(aFactionFilter.get(), aNew));
          // Don't consume event, others may listen for it too.
        });

    aFactionFilter.addListener(
        (aObs, aOld, aNew) -> {
  ...
  private void updateGraph() {
    ...
    for (final ChassisGroup selectionGroup :
        payloadChassis.getSelectionModel().getSelectedItems()) {
      ...
      for (final Chassis chassis : selectionGroup) {
        ...
        if (!consumed) {
          ...
          final PayloadStatistics statistics =
              new PayloadStatistics(
                  payloadXLEngine.isSelected(),
    ...
    FxGraphUtils.setTightBounds(
        payloadGraph.getXAxis(), payloadGraph.getYAxis(), 10.0, 5.0, payloadGraph.getData());
  }
}
