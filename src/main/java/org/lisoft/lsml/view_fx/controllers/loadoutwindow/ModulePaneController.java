...
public class ModulePaneController extends AbstractFXController {
  ...
  @FXML private FixedRowsListView<Consumable> consumablesView;
    ...
    if (data.isPresent()) {
      success =
          LiSongMechLab.safeCommand(
              root, stack, new CmdAddModule(messageDelivery, loadout, data.get()), messageDelivery);
    }
  ...
  public void onDragOver(DragEvent aDragEvent) {
    final Dragboard db = aDragEvent.getDragboard();
    EquipmentDragUtils.unpackDrag(db, Consumable.class)
        .ifPresent(
            aModule -> {
