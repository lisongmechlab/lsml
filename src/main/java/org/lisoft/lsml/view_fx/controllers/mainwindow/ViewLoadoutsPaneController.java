...
import org.lisoft.lsml.view_fx.controls.LoadoutPillCell;
...
public class ViewLoadoutsPaneController extends AbstractFXController implements MessageReceiver {
  ...
  @FXML private Region listingTypeIcon;
  @FXML private ListView<Loadout> loadoutPills;
  @FXML private TreeView<GaragePath<Loadout>> loadoutTree;
   ...
   */
  @Inject
  public ViewLoadoutsPaneController(
      Settings aSettings,
    ...
    garageFile.addListener((aObs, aOld, aNew) -> Platform.runLater(this::refreshAll));
  ...
  public void addGarageFolder() {
    final TreeItem<GaragePath<Loadout>> selectedItem =
    ...
    if (aMsg instanceof GarageMessage) {
      ...
      if (null != selectedDirectory && msg.path.isLeaf()) {
        if (msg.path
                .getValue()
                .orElseThrow(
                    () -> new RuntimeException("Expected received message to contain a path!"))
            instanceof Loadout) {
  ...
  public void refreshAll() {
    FxControlUtils.setupGarageTree(
        loadoutTree,
        globalGarage.getGarage().getLoadoutRoot(),
        xBar,
        commandStack,
        false,
        Loadout.class,
  ...
  private void refreshPills() {
    ...
    loadoutPills.setCellFactory(
        aView ->
            new LoadoutPillCell(settings, xBar, commandStack, loadoutTree, aView, loadoutFactory));
  ...
  private void updateAllLoadoutPills(GaragePath<Loadout> aNew) {
    ...
    if (null != aNew) {
      final SortedList<Loadout> sorted =
          new SortedList<>(
  ...
  private void updateListingIcon(
      ObservableValue<? extends Boolean> aObs, Boolean aOld, Boolean aNew) {
    if (null != aNew) {
