...
/**
 * This control is a text field that only allows manual edit trigger. If not editing then it behaves
 * as a label.
 *
 * @param <T> The type of the object that the name is a property of. Must extend {@link
 *     NamedObject}.
 * @author Li Song
 */
public class NameField<T extends NamedObject> extends StackPane {
    ...
    field.setOnAction(
        aEvent -> {
          if (!field.getText().equals(object.getName()) && path != null) {
            if (!LiSongMechLab.safeCommand(
                this, aStack, new CmdGarageRename<>(aMD, path, field.getText()), aMD)) {
   ...
   *
   * <p>If the object changes location in the garage, this method must be called to update.
   *
   * @param aObject The object to match the name to.
   * @param aGaragePath The path of the object in the garage. May be <code>null</code> if the object
   *     is not in a garage.
   */
