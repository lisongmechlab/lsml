 ...
 * @author Li Song
 */
public class GarageTreeItem<T extends NamedObject> extends TreeItem<GaragePath<T>>
    implements MessageReceiver {
  private final Class<T> clazz;
  private final boolean showValues;
  private final MessageReception xBar;

  public GarageTreeItem(
      MessageReception aXBar, GaragePath<T> aPath, boolean aShowValues, Class<T> aClazz) {
    ...
    if (!path.isLeaf()) {
      ...
      for (final GarageDirectory<T> child : topDirectory.getDirectories()) {
        getChildren()
            .add(new GarageTreeItem<>(xBar, new GaragePath<>(path, child), showValues, aClazz));
      }

      if (showValues) {
        for (final T value : topDirectory.getValues()) {
          getChildren()
              .add(new GarageTreeItem<>(xBar, new GaragePath<>(path, value), showValues, aClazz));
        }
      }
      sortChildren();
    }
    ...
    if (aMsg instanceof final GarageMessage<?> msg) {
      ...
      if (msg.type == GarageMessageType.ADDED) {
        ...
        if (!myPath.isLeaf() && msgPath.getParentDirectory() == myPath.getTopDirectory()) {
          ...
          Platform.runLater(
              () -> {
                // Because the msgPath's parent directory refers by identity to us, this cast is
                // safe.
            ...
            (aLHS, aRHS) -> {
              ...
              if (lhs.isLeaf() && rhs.isLeaf()) {
                return lhs.getValue()
                    .get()
                    .getName()
                    .compareToIgnoreCase(rhs.getValue().get().getName());
              }
              if (lhs.isLeaf() != rhs.isLeaf()) {
