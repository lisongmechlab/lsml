...
public class WeaponSummary {
   ...
   * @param aModifierSupplier A {@link Supplier} that provides modifiers to use for computing the
   *     values of the weapon.
   */
    ...
    ammoRounds =
        new DoubleBinding() {
          ...
          protected double computeValue() {
            ...
            return ammo.stream()
                    .collect(
                        Collectors.summingInt(ammunition -> ammunition.getNumRounds(modifiers)))
                + weapons.stream()
                    .map(w -> (AmmoWeapon) w)
        ...
        new StringBinding() {
          ...
          protected String computeValue() {
            if (weapons.isEmpty()) {
              ...
              return ammo.sorted(Comparator.comparingDouble(Ammunition::getMass).reversed())
                  .iterator()
                  .next()
                  .getShortName();
            }
            ...
            if (shouldCountTubes(weapon)) { // Implies missile weapon -> ammo weapon
              ...
              return aAmmoWeapon
                  .getShortName()
                  .replaceFirst("\\d+", Integer.toString(volleySize.get()));
            }
  ...
  /**
   * @return A {@link DoubleBinding} that calculates how long the weapon(s) represented by this
   *     {@link WeaponSummary} can be combat effective.
   */
   ...
   * @return <code>true</code> if the item was melded into this {@link WeaponSummary}. Returns
  ...
  /**
   * @return <code>true</code> if this {@link WeaponSummary} doesn't represent any active weapon or
   *     ammo and should be removed.
   */
  ...
  /**
   * @return A {@link StringProperty} that represents the display name of this {@link
   *     WeaponSummary}. Based on the volley size and weapon/ammo type.
   */
  ...
  /**
   * Attempts to split (or demeld) the given item from this {@link WeaponSummary} and updates this
   * in the process.
   *
   * @param aItem The item to attempt to split away.
   * @return <code>true</code> if the item was successfully split from this, <code>false</code>
   *     otherwise.
   */
   ...
   *     {@link WeaponSummary}. The property will have the value {@link Double#POSITIVE_INFINITY}
   *     for weapons that don't use ammo.
   */
  ...
  /**
   * @return A {@link DoubleBinding} that calculates how much total damage this {@link
   *     WeaponSummary} can do.
   */
  ...
  /**
   * @return A {@link IntegerProperty} that represents how many units of ammo, or rounds, are
   *     consumed on one alpha strike of this {@link WeaponSummary}. If the weapon doesn't use ammo,
   *     the volley size is the number of weapons that fire simultaneously. If this {@link
   *     WeaponSummary} doesn't have a weapon associated, i.e. only ammo without a matching weapon,
   *     then the volley size property will be <code>zero</code>.
   */
