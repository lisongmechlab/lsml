...
public class PayloadGrouping {
  ...
  public static class DataPoint {
    public final List<Chassis> chassis = new ArrayList<>();
  ...
  /**
   * Creates a new payload grouping based on the given representative chassis and payload
   * calculation class.
   *
  ...
  public void addToGraph(XYChart<Double, Double> aChart) {
    ...
    for (final DataPoint point : curve.points) {
      ...
      if (previousPoint != null
          && !ListArrayUtils.equalsUnordered(previousPoint.chassis, point.chassis)) {
  ...
  public boolean offer(Chassis aChassis) {
    ...
    if (aChassis instanceof final ChassisStandard chassisStd) {
      ...
      if (chassisRep.getMassMax() == chassisStd.getMassMax()
          && // Must have same mass
          getSpeedFactor(chassisRep) == getSpeedFactor(chassisStd)
          && // Must have same speed factor
          (!payloadStatistics.isFerroFibrous()
              || chassisRep.getFaction() == chassisStd.getFaction())) {
      ...
      if (chassisRep.getMassMax() == chassisOmni.getMassMax()
          && // Must have same mass
          chassisRep.getFixedEngine() == chassisOmni.getFixedEngine()
          && // Must have same engine
          getSpeedFactor(chassisRep) == getSpeedFactor(chassisOmni)
          && // Must have same speed factor
  ...
  private String makeGroupName(Collection<Chassis> aChassisGroup) {
    ...
    for (final Entry<String, List<Chassis>> series : bySeries.entrySet()) {
      ...
      for (final Chassis chassis : allInSeries) {
        ...
        while (i < seriesName.length()
            && i < name.length()
            && seriesName.charAt(i) == name.charAt(i)) {
          i++;
        }
