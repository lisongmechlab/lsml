...
public class StyleManager {
  ...
  public static void changeListStyle(Node aNode, EquipmentCategory aCategory) {
    if (aCategory != null) {
      ...
      aNode
          ...
          .removeIf(
              clazz ->
                  clazz.startsWith("equipment")
                      && !clazz.equals(CLASS_EQ_LIST)
    ...
    if (aEquipment != null) {
      ...
      if (EquipmentCategory.MISC == category) {
        if (aEquipment instanceof JumpJet) {
          ...
          if (aEquipment == ItemDB.DYN_ARMOUR
              || aEquipment == ItemDB.DYN_STRUCT
              || aEquipment == ItemDB.FIX_ARMOUR
              || aEquipment == ItemDB.FIX_STRUCT) {
  ...
  public static Node makeMissileBayDoorsIcon() {
    final Region r = new Region();
    r.getStyleClass().add("svg-missile-bay-doors");
    r.getStyleClass().add("icon");
    r.getStyleClass().add(CLASS_ICON_SMALL);
    r.setStyle("-fx-background-color: -fx-text-background-color;");
    return r;
  }
