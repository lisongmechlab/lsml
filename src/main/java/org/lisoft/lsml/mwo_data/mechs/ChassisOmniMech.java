...
/**
 * This class models an omnimech chassis, i.e. the basic attributes associated with the chassis and
 * the center omnipod.
 *
 * @author Li Song
 */
public class ChassisOmniMech extends Chassis {
  ...
  public ChassisOmniMech(
      ...
      Faction aFaction,
      ComponentOmniMech[] aComponents,
      StructureUpgrade aStructureType,
      ArmourUpgrade aArmourType,
      HeatSinkUpgrade aHeatSinkType,
  ...
  /**
   * @return The number of heat sinks that are fixed on this chassis, including the ones in the
   *     fixed engine.
   */
  ...
  /**
   * @return The {@link MovementProfile} where the {@link OmniPod} for each {@link
   *     ComponentOmniMech} is selected to maximize each attribute. All the values of the {@link
   *     MovementProfile} may not be attainable simultaneously but each value of each attribute is
   *     independently attainable for some combination of {@link OmniPod}.
   */
  ...
  /**
   * @return The {@link MovementProfile} where the {@link OmniPod} for each {@link
   *     ComponentOmniMech} is selected to minimize each attribute. All the values of the {@link
   *     MovementProfile} may not be attainable simultaneously but each value of each attribute is
   *     independently attainable for some combination of {@link OmniPod}.
   */
  ...
  public Collection<Modifier> getStockModifiers() {
    return Arrays.stream(Location.values())
        .map(location -> OmniPodDB.lookupStock(this, location))
        .filter(Optional::isPresent)
        .flatMap(pod -> pod.get().getQuirks().stream())
  ...
  public boolean isAllowed(Item aItem) {
    ...
    return super.isAllowed(
        aItem); // Anything else depends on the actual combination of omnipods equipped
  }
