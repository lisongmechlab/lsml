...
/**
 * This movement profile gives the maximum possible value of a combination of different movement
 * profiles.
 *
 * <p>More specifically, a base profile is defined (typically chassis standard values) and a number
 * of alternatives are given in groups. For each group the highest value among the alternatives is
 * taken and added to the base profile.
 *
 * <p>Movement archetype is always that of the base profile.
 *
 * @author Li Song
 */
  ...
  protected double calc(String aMethodName, Collection<Modifier> aExtraModifiers) {
    try {
      double baseValue =
          (double)
              base.getClass()
                  .getMethod(aMethodName, Collection.class)
      ...
      for (List<Collection<Modifier>> group : groups) {
        ...
        for (Collection<Modifier> quirks : group) {
          ...
          double value =
              (double)
                  base.getClass().getMethod(aMethodName, Collection.class).invoke(base, fullQuirks);
