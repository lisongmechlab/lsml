...
public class CmdSetArmourType extends CmdUpgradeBase {
  ...
  /**
   * Creates a new {@link CmdSetStructureType} that will change the armour type of a {@link
   * LoadoutStandard}.
   *
   ...
   */
  public CmdSetArmourType(
      MessageDelivery aMessageDelivery, LoadoutStandard aLoadout, ArmourUpgrade aArmourUpgrade) {
  ...
  /**
   * Creates a {@link CmdSetArmourType} that only affects a stand-alone {@link UpgradesMutable}
  ...
  protected void set(ArmourUpgrade aNew, ArmourUpgrade aOld) throws EquipException {
    if (aNew != aOld) {
      ...
      if (aNew == UpgradeDB.IS_STEALTH_ARMOUR
          && loadout.getItemsOfHardPointType(HardPointType.ECM) < 1) {
