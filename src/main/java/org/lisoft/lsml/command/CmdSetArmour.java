...
public class CmdSetArmour implements Command {
    ...
    private void operationTryToLegalize() throws EquipException {
        ...
        if (freeTonnage < 0) {
            // See if the armour can be freed from a combination of automatic components. They will be redistributed
            ...
            if (manual == true && freed < armourDiff) {
                for (final ConfiguredComponent otherComponent : loadout.getComponents()) {
                    if (component != otherComponent && !otherComponent.hasManualArmour()) {
                        ...
                        for (final ArmourSide armourSide : ArmourSide.allSides(otherComponent.getInternalComponent())) {
