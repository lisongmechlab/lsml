 ...
 * ton) on a loadout, respecting manually set values.
 *
 * @author Li Song
 */
public class CmdDistributeArmour extends CompositeCommand {
   ...
   */
  public CmdDistributeArmour(
      Loadout aLoadout,
      int aPointsOfArmour,
      double aFrontRearRatio,
  ...
  private void applyArmours(
      Loadout aLoadout, double aFrontRearRatio, MessageDelivery aMessageDelivery) {
    ...
    for (final Location part : Location.values()) {
      ...
      if (loadoutPart.getInternalComponent().getLocation().isTwoSided()) {
        ...
        addOp(
            new CmdSetArmour(
                aMessageDelivery, loadout, loadoutPart, ArmourSide.FRONT, front, false));
        addOp(
            new CmdSetArmour(aMessageDelivery, loadout, loadoutPart, ArmourSide.BACK, back, false));
      } else {
        addOp(
            new CmdSetArmour(
                aMessageDelivery, loadout, loadoutPart, ArmourSide.ONLY, armour, false));
      }
    }
  }
  ...
  private int calculateArmourToDistribute(Loadout aLoadout, int aPointsOfArmour) {
    ...
    final double expectedLoadoutMass =
        Math.floor((unarmouredMass + requestedArmourMass) / MASS_QUANTA) * MASS_QUANTA;
    ...
    final int maxArmourTonnage =
        (int)
            ((aLoadout.getChassis().getMassMax() - unarmouredMass)
  ...
  private void distribute(
      final Loadout aLoadout, int aArmourAmount, final Map<Location, Integer> aPriorities) {
    int prioSum = 0;
    for (final double prio : aPriorities.values()) {
      prioSum += prio;
    }

    final SortedMap<Location, Integer> byPriority =
        new TreeMap<>(
            (aO1, aO2) -> {
              ...
              if (c == 0) {
                final int d =
                    Integer.compare(
                        aLoadout.getComponent(aO1).getInternalComponent().getArmourMax(),
    ...
    for (final Entry<Location, Integer> entry : byPriority.entrySet()) {
      ...
      final ConfiguredComponent loadoutPart = aLoadout.getComponent(part);
      final int armour =
          Math.min(loadoutPart.getInternalComponent().getArmourMax(), armourLeft * prio / prioSum);
