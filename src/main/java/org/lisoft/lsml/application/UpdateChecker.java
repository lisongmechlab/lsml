...
/**
 * This class will connect to the GitHub repository and see if there is an
 * update.
 *
 * @author Emily Björk
 */
public class UpdateChecker {
	...
	private static List<ReleaseData> parse(InputStream is) {
		...
		final Pattern tag_pattern = Pattern.compile("\"([^\"]+)\"\\s*:\\s*(?:(?:\"([^\"]*)\")|(?:(\\w+)))");
	...
	/**
	 * Creates a new {@link UpdateChecker} that will check for updates in the
	 * background and call the callback when it is done.
	 *
	 * @param aUrl
	 ...
	 * @param aAcceptBeta
	 *            <code>true</code> if beta releases should be considered as an
	 *            update.
	 */
