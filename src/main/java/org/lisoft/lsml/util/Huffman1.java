...
public class Huffman1<T> {
     ...
     * <p>
     * If the encoded bit stream is not an even multiple of 8 bits, the encoder fills the remaining bits in the last
     * byte with a stop symbol. If the stop symbol's codeword didn't fit into the remaining bits, a partial stop symbol
     * is encoded and no extra byte is emitted. When decoding the stream and the decoder encounters a fully encoded stop
    ...
    public Huffman1(Map<T, Integer> aSymbolFrequencyTable, T aStopSymbol) throws IllegalArgumentException {
        ...
        for (final Map.Entry<T, Integer> pair : aSymbolFrequencyTable.entrySet()) {
            ...
            final Leaf<T> leaf = new Leaf<>(pair.getKey(), pair.getValue());
            forest.offer(leaf);
        ...
        stopLeaf = new Leaf<>(aStopSymbol, 0);
