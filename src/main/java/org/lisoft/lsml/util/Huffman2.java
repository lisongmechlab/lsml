 ...
 *
 * <p>Compared to {@link Huffman1} this class defines a strict, stable ordering of nodes in the tree
...
public class Huffman2<T extends Comparable<T>> {
   ...
   *
   * <p>The top symbol is treated specially. It must be a symbol that does not occur in the
   * frequency table and it may be <code>null</code> iff <code>null</code> is not in the frequency
   * table.
   *
   * <p>If the encoded bit stream is not an even multiple of 8 bits, the encoder fills the remaining
   * bits in the last byte with a stop symbol. If the stop symbol's codeword didn't fit into the
   * remaining bits, a partial stop symbol is encoded and no extra byte is emitted. When decoding
   * the stream and the decoder encounters a fully encoded stop symbol, it stops the decoding and
   * returns the decoded data up to the stop symbol. If the stop symbol was only partially encoded
   * the decoder will stop before fully decoding the symbol. In both cases the output has the
   * correct number of symbols.
   *
   * @param aSymbolFrequencyTable A {@link Map} where each symbol <code>T</code> has a frequency
   *     associated with it. These frequencies are used to generate symbol probabilities.
   * @param aStopSymbol The special symbol to use as a stop symbol. Must not be represented in
   *     aSymbolFrequencyTable. Can be <code>null</code> iff <code>null</code> is not represented in
   *     aSymbolFrequencyTable.
   * @throws IllegalArgumentException Thrown if <code>aStopSymbol</code> exists in the <code>
   *     aSymbolFrequencyTable</code> {@link Map}.
   */
  public Huffman2(Map<T, Integer> aSymbolFrequencyTable, T aStopSymbol)
    ...
    final List<Map.Entry<T, Integer>> frequencies =
        new LinkedList<>(aSymbolFrequencyTable.entrySet());
    frequencies.sort(Map.Entry.comparingByKey()); // Order frequencies by
    ...
    final PriorityQueue<Node> forest =
        new PriorityQueue<>(
            aSymbolFrequencyTable.size(),
    ...
    // Some implementations of map do not allow null as a key, and stopLeaf can be null so don't add
    // it to leaves.
   ...
   * @throws EncodingException Thrown if <code>aSymbolList</code> contains a symbol which doesn't
   *     exist in the frequency table given to the constructor.
   */
  public byte[] encode(List<T> aSymbolList) throws EncodingException {
    ...
    for (int i = 0; i < aSymbolList.size() + 1; ++i) {
      final T symbol;
      final Leaf<T> node;
      // Detect if we're coding the stop symbol, it is not in the leafs map as it may have a null
      // symbol.
      ...
      while (prefixBitsLeft > 0) {
        ...
        if (bits == Byte.SIZE) {
          bits = 0;
          bytes++;
          // If we're encoding the stop symbol, don't emit any more bytes as we are sure to not get
          // junk when
          // decoding anyway.
          if (node == stopLeaf) {
            break;
  ...
  /**
   * Determines a buffer size that will be large enough to hold the result from encoding the given
   * list of symbols with this encoder. This method basically takes twice the result of Shannon's
   * source coding theorem.
   *
   * @param aSymbolList The list of symbols that should be encoded into the buffer.
   * @return An <code>int</code> with number of <code>byte</code>s the intermediary buffer needs to
   *     be.
   */
