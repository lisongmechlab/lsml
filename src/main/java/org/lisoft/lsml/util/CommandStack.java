...
/**
 * This class models an command stack that can be used for undo etc (see: Command Pattern). It will
 * automatically reset the stack if a new garage is loaded.
 *
 * @author Li Song
 */
public class CommandStack {
  /**
   * This class models an operation that should be considered as one but actually consists of many
   * smaller operations that are all performed in order as one transaction.
   *
   * @author Li Song
   */
  public abstract static class CompositeCommand implements Command {
    ...
    public boolean equals(Object obj) {
      ...
      if (!(obj instanceof final CompositeCommand other)) {
        return false;
      }
    ...
    /**
     * The user should implement this to create the operation. Will be called only once, immediately
     * before the first time the operation is applied.
     *
   ...
   * resets and has the given undo depth.
   *
  ...
  /**
   * The {@link Command} class represents an action that can be (un)done. Undoing the action will
   * restore the state of affected object to that before the {@link Command} was done.
   *
   * @author Li Song
   */
  public interface Command {
    ...
    /**
     * Checks if two operations can be coalesced into one. By definition an object can't coalesce
     * with itself.
     *
     * <p>If this function returns true, then the previous operation may be quietly undone and this
     * operation replace it. I.e. premises for the operation to succeed may have changed from
     * construction time to the time point when apply is called.
     *
