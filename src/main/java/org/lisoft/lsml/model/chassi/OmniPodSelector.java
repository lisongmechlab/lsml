...
/**
 * Determines {@link OmniPod}s for a {@link LoadoutOmniMech} so that the given
 * requirements on hard points is met.
 *
 * @author Emily Björk
 */
public class OmniPodSelector {
	private static class PartialSelection implements PartialCandidate<PartialSelection> {
		...
		/**
		 * Creates a root partial selection to start searching for. Also defines
		 * the constraints.
		 *
		 * @param aAllowedPods
		 *            The pods that are allowed to be selected (preferably (but
		 *            not necessarily) pruned from pods that do not affect the
		 *            constraints).
		 * @param aWantedEnergy
