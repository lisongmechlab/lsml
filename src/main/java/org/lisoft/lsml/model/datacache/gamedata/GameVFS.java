...
/**
 * This class is a Virtual File System for finding data files in the game
 * folder.
 *
 * @author Li Song
 */
public class GameVFS {
	/**
	 * This structure contains information about a game file, its CRC, path and
	 * a stream for reading from it.
	 *
	 * @author Li Song
	 */
	...
	/**
	 * Checks if the data files necessary to start LSML are available. If false
	 * is returned then either {@link Settings#CORE_FORCE_BUNDLED_DATA} property
	 * must be set to true or {@link Settings#CORE_GAME_DIRECTORY} must be set
	 * to refer to a valid game directory.
	 *
	 * @return <code>true</code> if necessary information is available to start
	 *         LSML.
	 */
	public static boolean areDataFilesAvailable(Settings aSettings) {
		final Property<Boolean> forceBundled = aSettings.getBoolean(Settings.CORE_FORCE_BUNDLED_DATA);
	...
	/**
	 * Attempts a (smart) search of all file system roots to automatically
	 * detect a MWO installation.
	 *
	 * @param aCurrentFileReport
	 *            A {@link StringProperty} that will be set to the currently
	 *            searched directory to provide feedback.
	 * @param aConfirmationCallback
	 *            Called with a {@link Path} when a candidate location is found,
	 *            the callback should confirm with the user if this is indeed
	 *            the correct installation (in the case of multiple MWO
	 *            installations such as PTS).
	 * @return <code>true</code> if the {@link Settings#CORE_GAME_DIRECTORY}
	 *         property has been updated to a valid installation.
	 */
	...
	/**
	 * Determine if the given {@link Path} points to the root of a valid game
	 * install.
	 *
	 * @param aFile
	 *            The path to check.
	 * @return <code>true</code> if <code>aPath</code> points to a valid game
	...
	/**
	 * @return A {@link List} of {@link Path}s that are likely to contain the
	 *         game.
	 */
	...
	/**
	 * Creates a new virtual file system for game files in the given directory
	 * which must be a valid game install. See
	 ...
	 * <p>
	 * NOTE: This currently only works with paths that are not inside of
	 * archives
	 * </p>
	 *
	 * @param aPath
	 ...
	 *            The path to the file to open, with archive file names
	 *            expanded. For example "Game/Objects/mechs/spider/sdr-5k.mdf"
