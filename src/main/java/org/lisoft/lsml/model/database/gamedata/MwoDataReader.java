 ...
 * files and produces a usable {@link Database} file.
 *
 * @author Emily Björk
 */
public class MwoDataReader {
	...
	public Optional<Database> parseGameFiles(Writer aLog, File aGameDirectory) throws Exception {
		try {
			...
			Localisation.initialize(gameVFS); // FIXME: Make localisation into an object and inject it into the reader.
	...
	/**
	 * Compares the database to the game files and determines if there is any reason
	 * to attempt a further parse.
	 *
	 * @param aDatabase
	 *            The {@link Database} to compare to.
	 * @param aGameDirectory
	 *            The directory to read game files to compare to.
	 * @return <code>true</code> if the game files have newer data than what's in
	 *         the database.
	 */
	...
	/**
	 * Parses all inner sphere {@link ChassisStandard} from the ItemStats.xml file
	 * and related files.
	 *
	 * @param aGameVfs
