...
public class MwoDataReader {
  ...
  public static OmniPod findOmniPod(List<OmniPod> aOmniPods, int aOmniPod) {
  ...
  private List<Chassis> parseChassis(
      ...
      GameVFS aGameVFS)
      throws IOException {
    final XMLMechIdMap mechIdMap =
        XMLMechIdMap.fromXml(aGameVFS.openGameFile(GameVFS.MECH_ID_MAP_XML).stream);
    ...
    for (final XMLItemStatsMech mech : aItemStatsXml.MechList) {
      try {
        ...
        final MdfMechDefinition mdf =
            MdfMechDefinition.fromXml(
        ...
        if (mdf.isOmniMech()) {
          ...
          final XMLHardpoints hardPoints =
              XMLHardpoints.fromXml(
                  aGameVFS.openGameFile(new File(GameVFS.MDF_ROOT, hardPointsXml)).stream);
          ans.add(
              mdf.asChassisStandard(mech, aId2obj, aModifierDescriptors, mechIdMap, hardPoints));
        }
  ...
  private List<Environment> parseEnvironments(Writer aLog, GameVFS aGameVFS) throws IOException {
    ...
    for (final File file : levels) {
      ...
      final Mission mission =
          (Mission)
              xstream.fromXML(aGameVFS.openGameFile(new File(file, "mission_mission0.xml")).stream);
      ...
      if (!found) {
        ...
        if (aLog != null) {
          aLog.append("Unable to load temprature for level: ")
              .append(uiName)
              .append("! Assuming 0.0.")
   ...
   * @throws Exception
   */
  private List<Consumable> parseModules(XMLItemStats aItemStatsXml, GameVFS aGameVFS)
      throws Exception {
        ...
        case "CUAVStats":
          {
            ans.add(
                new Consumable(
                    statsModule.getUiName(),
                    statsModule.getUiShortName(),
                    statsModule.getUiDescription(),
                    statsModule.getMwoKey(),
                    statsModule.getMwoId(),
    ...
    for (final Chassis chassis : aChassis) {
      ...
      try {
        ...
        throw new ParseErrorException(
            "Error while parsing stock loadout for: " + chassis.getName() + " from: " + loadoutXml,
            e);
      }
    }
    return ans;
        ...
        case ARMOUR:
          {
            ...
            ans.add(
                new ArmourUpgrade(
                    ...
                    slots,
                    armourPerTon,
                    upgradeType.getFixedSlotsPerComponent().orElse(null),
        ...
        case HEATSINK:
          {
            final HeatSink heatSink =
                (HeatSink) id2obj.get(upgradeType.HeatSinkTypeStats.compatibleHeatSink);
