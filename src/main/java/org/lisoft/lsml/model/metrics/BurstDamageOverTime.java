...
public class BurstDamageOverTime extends RangeTimeMetric implements MessageReceiver {
  ...
  /**
   * Creates a new {@link BurstDamageOverTime} metric that calculates the maximal burst damage using
   * all weapons on the loadout.
   *
  ...
  /**
   * Creates a new {@link BurstDamageOverTime} that only calculates the damage for the given weapon
   * group.
   *
   * @param aLoadout The loadout to calculate for.
   * @param aReception The crossbar to listen to changes on the loadout on.
  ...
  private void updateEvents(double aRange) {
    ...
    for (final Weapon weapon : weapons) {
      ...
      if (weapon instanceof final EnergyWeapon energyWeapon) {
        if (energyWeapon.getDuration(modifiers) > 0) {
          damageIntegrals.add(
              new IntegratedPulseTrain(
                  period,
                  energyWeapon.getDuration(modifiers),
                  damage / energyWeapon.getDuration(modifiers)));
          continue;
        }
      } else if (weapon instanceof final BallisticWeapon ballisticWeapon) {
        if (ballisticWeapon.canJam()) {
          final double range =
              aRange < 0.0 ? ballisticWeapon.getRangeOptimal(modifiers).minimum : aRange;
