...
public class GhostHeat implements Metric {
  private static final double[] HEAT_SCALE = {
  ...
  /**
   * Constructs a new {@link GhostHeat} metric that will calculate the ghost heat for the entire
   * loadout.
   *
  ...
  /**
   * Constructs a new {@link GhostHeat} metric that calculates the ghost heat for a given weapon
   * group.
   *
   * @param aLoadout The loadout to calculate for.
  ...
  public double calculate() {
    ...
    for (final Entry<Weapon, Integer> entry : ungroupedWeapons.entrySet()) {
      ...
      penalty +=
          calculatePenalty(
              ...
              weapon.getGhostHeatMultiplier(),
              weapon.getGhostHeatMaxFreeAlpha(modifiers),
              count);
    }
    ...
    for (final List<Weapon> group : groups.values()) {
      ...
      double maxMultiplier =
          group.stream().map(Weapon::getGhostHeatMultiplier).max(naturalOrder()).get();
      @SuppressWarnings("OptionalGetWithoutIsPresent") // Always present
      int maxFreeAlpha =
          group.stream().map(w -> w.getGhostHeatMaxFreeAlpha(modifiers)).min(naturalOrder()).get();
