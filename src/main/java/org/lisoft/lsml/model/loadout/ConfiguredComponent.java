 ...
 *
 * <p>This class is immutable. The only way to alter it is by creating instances of the relevant
 * {@link Command}s and adding them to an {@link CommandStack}.
 *
 * @author Li Song
 */
public abstract class ConfiguredComponent {
  ...
  public ConfiguredComponent(Component aInternalComponent, boolean aManualArmour) {
    ...
    for (final ArmourSide side : ArmourSide.allSides(internalComponent)) {
      armour.put(
          ...
          new Attribute(
              0,
              ModifierDescription.SEL_ARMOUR_RESIST,
              ModifierDescription.specifierFor(internalComponent.getLocation(), side)));
    }
  }

  /**
   * Copy constructor. Performs a deep copy of the argument with a new {@link LoadoutStandard}
   * value.
   *
  ...
  public ConfiguredComponent(ConfiguredComponent aComponent) {
    ...
    for (final Map.Entry<ArmourSide, Attribute> e : aComponent.armour.entrySet()) {
      armour.put(
          e.getKey(),
          new Attribute(
              e.getValue().value(null), e.getValue().getSelectors(), e.getValue().getSpecifier()));
    }

    items.addAll(aComponent.items);
  }

  /**
   * Adds a new item to this component. This method does not verify loadout invariants and can put
   * the component into an illegal state. It is the caller's responsibility to make sure local and
   * global conditions are met before adding an item.
   *
   * <p>This is intended for use only from {@link CmdAddItem}, {@link CmdRemoveItem} and relatives.
   *
   * <p>Please note that {@link #canEquip(Item)} must return true prior to a calling this.
   *
   * @param aItem The item to add.
   * @return The index where the item was added or -1 if the item was consumed by another item (HS
   *     going into engine for example).
   */
  ...
  /**
   * Checks if all local conditions for the item to be equipped on this component are fulfilled.
   * Before an item can be equipped, global conditions on the loadout must also be checked by {@link
   * Loadout#canEquipDirectly(Item)}.
   *
  ...
  public EquipResult canEquip(Item aItem) {
    ...
    if (aItem.getHardpointType() != HardPointType.NONE
        && getItemsOfHardpointType(aItem.getHardpointType())
            >= getHardPointCount(aItem.getHardpointType())) {
      return EquipResult.make(
          getInternalComponent().getLocation(), EquipResultType.NoFreeHardPoints);
    }
   ...
   *
   * @param aItem The item to check if it can be removed.
  ...
  public boolean equals(Object aObject) {
    ...
    if (!(aObject instanceof final ConfiguredComponent that)) {
      return false;
    }
   ...
   *
   * @param aArmourSide The {@link ArmourSide} to query. Querying the wrong side results in a {@link
  ...
  /**
   * Will return the number of armour points that can be set on the component. Taking both armour
   * sides into account and respecting the max armour limit. Does not take free tonnage into
   * account.Querying the wrong side results in a {@link IllegalArgumentException}.
   *
   ...
   * @param aArmourSide The {@link ArmourSide} to query. Querying the wrong side results in a {@link
   *     IllegalArgumentException} .
   * @param aModifiers A {@link Collection} of {@link Modifier}s to use for calculating the actual
   *     armour amount.
   * @return The current amount of armour on the given side of this component.
   */
  ...
  /**
   * @return The number of heat sinks inside the engine (if any) equipped on this component. Does
   *     not count the (up to) 10 included in the engine itself, rather it only counts the external
   *     heat sink slots.
   */
  ...
  /**
   * @return The maximal number of heat sinks that the engine (if any) equipped on this component
   *     can sustain.
   */
  ...
  /**
   * @return The number of critical slots locally available on this component. Note: may be less
   *     than globally available slots as this doesn't take floating slots (such as dynamic armour
   *     on standard mechs) into account.
   */
  ...
  /**
   * @return The number of critical slots that are used in this component, not counting floating
   *     slots used by dynamic armour or structure.
   */
  public int getSlotsUsed() {
    int slotsUsed = getInternalComponent().getFixedItemSlots();
    ...
    for (final Item item : items) {
      ...
      slotsUsed += item.getSlots();
    }
    return slotsUsed;
  }

  /**
   * @return <code>true</code> if this component's armour has been set manually (otherwise it's been
   *     set automatically).
   */
  ...
  /**
   * @return <code>true</code> if this component has missile bay doors, <code>false</code>
   *     otherwise.
   */
  ...
  public int hashCode() {
    final int prime = 31;
    int result = 1;
    result = prime * result + armour.hashCode();
