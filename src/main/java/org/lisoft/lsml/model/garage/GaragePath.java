...
public class GaragePath<T> {
  ...
  private final GaragePath<T> parent;
   ...
   */
  public static <T> GaragePath<T> fromPath(String aString, GarageDirectory<T> aRoot)
      throws IOException {
    ...
    for (final Iterator<String> it = components.iterator(); it.hasNext(); ) {
      ...
      if (!found) {
        if (it.hasNext()) {
          // We didn't find the path component we looked for and we were not on the last part of the
          // path
   ...
   *
   * <p>In other words it checks if it is safe to add a entry (value or directory) with the given
   * name, under the given path.
   *
  ...
  public boolean equals(Object obj) {
    if (obj instanceof final GaragePath<?> garagePath) {
  ...
  private static String escape(String aString) {
    ...
    return aString
        .replace(String.valueOf(ESCAPE_SYMBOL), "" + ESCAPE_SYMBOL + ESCAPE_SYMBOL)
