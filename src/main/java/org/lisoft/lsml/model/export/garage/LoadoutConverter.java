...
public class LoadoutConverter implements Converter {
   ...
   * @param aLoadoutFactory A {@link LoadoutFactory} that is used to provide loadout objects that
   *     the data is loaded into.S
   * @param aBuilder A {@link LoadoutBuilder} to report errors to.
   */
  public LoadoutConverter(
  ...
  @Override
  public void marshal(
      Object aObject, HierarchicalStreamWriter aWriter, MarshallingContext aContext) {
  ...
  private Loadout parseV1(HierarchicalStreamReader aReader, UnmarshallingContext aContext) {
    ...
    while (aReader.hasMoreChildren()) {
      ...
      if ("upgrades".equals(aReader.getNodeName())) {
        ...
        aContext.convertAnother(
            loadout,
            ConfiguredComponentStandard.class,
      ...
      if ("upgrades".equals(aReader.getNodeName())) {
        if (loadout instanceof LoadoutStandard) {
          ...
          while (aReader.hasMoreChildren()) {
            ...
            if (aReader.getNodeName().equals("guidance")) {
              try {
                final GuidanceUpgrade artemis =
                    (GuidanceUpgrade) UpgradeDB.lookup(Integer.parseInt(aReader.getValue()));
        ...
        while (aReader.hasMoreChildren()) {
          ...
          if (!"module".equals(aReader.getNodeName())) {
            throw new RuntimeException(
                "Malformed XML! Expected <module> got: " + aReader.getNodeName());
          }
