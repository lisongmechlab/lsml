...
/**
 * This class handles conversions of {@link LoadoutStandard}s to and from Base64 strings. It will
 * correctly determine which format the string is in and choose the right combination of decoders.
 *
 * @author Li Song
 */
@Singleton
public class Base64LoadoutCoder {
  ...
  public Base64LoadoutCoder(
      Encoder aBase64Encoder,
      Decoder aBase64Decoder,
      LoadoutCoderV2 aCoderV2,
      LoadoutCoderV3 aCoderV3,
      LoadoutCoderV4 aCoderV4) {
  ...
  public String encodeHTTPTrampoline(Loadout aLoadout) {
    try {
      ...
      return LSML_TRAMPOLINE + URLEncoder.encode(data, StandardCharsets.UTF_8);
  ...
  public Loadout parse(String aUrl) throws Exception {
    ...
    if (urlLowerCase.startsWith(LSML_PROTOCOL)) {
      ...
      url = URLDecoder.decode(url.substring(LSML_TRAMPOLINE.length()), StandardCharsets.UTF_8);
    }
