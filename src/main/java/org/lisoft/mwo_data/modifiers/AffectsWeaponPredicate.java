...
/**
 * This {@link Predicate} will return <code>true</code> if the tested {@link Modifier} will affect
 * the performance of any weapon.
 *
 * @author Li Song
 */
public class AffectsWeaponPredicate implements Predicate<Modifier> {
  private static final List<String> HEAT_SELECTORS;
  private static final Set<String> WEAPON_SELECTORS;
  private static final Set<String> WEAPON_SPECIFIERS;
  ...
  static {
    ...
    HEAT_SELECTORS.addAll(ModifierDescription.SEL_HEAT_EXTERNAL_TRANSFER);

    WEAPON_SELECTORS =
        ItemDB.lookup(Weapon.class).stream()
  ...
  public boolean test(Modifier aModifier) {
    ...
    if (!Collections.disjoint(selectors, ModifierDescription.SEL_ALL)
      ...
      return !Collections.disjoint(
          HEAT_SELECTORS, selectors); // Selects heat dissipation of entire mech
    }
  }
}
