...
public class ItemComparatorTest {
  ...
  public void testJumpJets() {
    ...
    for (int i = 1; i < jumpJets.size(); i++) {
      ...
      assertTrue(
          "Min tons of previous: " + prevMinTons + " min tons of current: " + currMinTons,
  ...
  @Test
  public void testSortWeaponsByRange_BreakTies() throws Exception {
    Weapon lrm15 = (Weapon) ItemDB.lookup("LRM 15");
    Weapon lrm20 = (Weapon) ItemDB.lookup("LRM 20");

    Comparator<Weapon> cut = ItemComparator.byRange(null);

    // LRM 15 < LRM 20, tie broken name
    assertEquals(-1, cut.compare(lrm15, lrm20));
  }

  @Test
  public void testSortWeaponsByRange_DistinctWeaponsNeverEqual() {
    // Two distinct weapons must never compare equal, otherwise maps and hash tables can break down.
    List<Weapon> weapons = ItemDB.lookup(Weapon.class);

    Comparator<Weapon> cut = ItemComparator.byRange(null);
    for (Weapon leftWeapon : weapons) {
      for (Weapon rightWeapon : weapons) {
        if (leftWeapon == rightWeapon) {
          assertEquals(0, cut.compare(leftWeapon, rightWeapon));
        } else {
          assertNotEquals(0, cut.compare(leftWeapon, rightWeapon));
        }
      }
    }
  }
