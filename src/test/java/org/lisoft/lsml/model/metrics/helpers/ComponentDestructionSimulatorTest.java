...
public class ComponentDestructionSimulatorTest {
  ...
  public void testDoubleCritToDestroy() {
    ...
    final double P_hit =
        CriticalStrikeProbability.CRITICAL_HIT_CHANCE.get(1)
            + CriticalStrikeProbability.CRITICAL_HIT_CHANCE.get(2);
   ...
   *
   * <p>We take 5 shots and compute the chance that it is destroyed.
   */
  @Test
  public void testSingleCritToDestroy() {
    ...
    // There are 5 shots before the item explodes, thus the probability of all of them missing the
    // item is:
  ...
  private Item makeTestItem(int aSlots, double aHP) {
    return new Item(
        null, null, null, 0, aSlots, 0, HardPointType.NONE, aHP, Faction.INNERSPHERE, null, null);
  }
}
