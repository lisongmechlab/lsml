...
public class ItemEffectiveHPTest {
	...
	/**
	 * The effective HP of an {@link Item} is the amount of damage that has to be
	 * dealt to the component the {@link Item} is housed in before the {@link Item}
	 * breaks. The actual effective HP is very dependent on the projectile damage
	 * and very stochastic in nature. However under the assumption that the damage
	 * is applied in infinitesimal chunks we can calculate the mean effective HP.
	 * This is an approximation of the effective HP for lasers and other weapons
	 * with a low projectile damage with high projectile count.
	 * <p>
	 * Easy case. When there is only one item, the effective HP is the item HP
	 * divided by the weighted (by multiplicity) chance to critically hit. See
	 * {@link CriticalItemDamage} for how much damage one damage to the component
	 ...
	 * After one of the components has reached it's maximum effective HP, it is
	 * destroyed and removed from the crit rolls so this affects other components
	 * effective HP.
	 */
	@Test
