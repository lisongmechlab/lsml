...
public class HeatGenerationTest {
  ...
  /**
   * Heat generation shall include heat per second from all weapons as well as the base heat from
   * the engine. But no heat from the jump jets.
   */
  @Test
  public void testCalculate() throws Exception {
    ...
    final double expected =
        ppc.getStat("h/s", null)
            + ll.getStat("h/s", null)
            + lrm20.getStat("h/s", null)
            + lb10x.getStat("h/s", null)
            + engine.getHeat(null);
  ...
  public void testCalculate_WeaponGroups() throws Exception {
    ...
    // Verify
    final double expected =
        ppc.getStat("h/s", null) + ll.getStat("h/s", null) + engine.getHeat(null);
