...
public class PayloadStatisticsTest {
  ...
  private double expectedMass(
      int aMaxMass, double aStructureMass, double aArmourMass, double aEngineMass, int aFixedHs) {
    final double mass = aStructureMass + aArmourMass + aEngineMass + Math.max(0, 10 - aFixedHs);
    return aMaxMass - mass;
  }

  private ChassisOmniMech makeOmniChassis(
      int maxMass,
      int maxArmour,
      int engineHs,
      double aStructureMass,
      double aArmourMass,
      double aEngineMass) {
  ...
  private void verifyOmniMech(
      final double structureMass,
      final double armourMass,
      final double engineMass,
      final int maxMass,
      final int maxArmour,
      final int fixedHs,
      boolean useXlEngine,
    ...
    // Setup
    final ChassisOmniMech chassis =
        makeOmniChassis(maxMass, maxArmour, fixedHs, structureMass, armourMass, engineMass);
