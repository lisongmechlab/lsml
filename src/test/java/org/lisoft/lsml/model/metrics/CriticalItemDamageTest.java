...
public class CriticalItemDamageTest {
  ...
  /**
   * Easy case. When there is only one item, the chance to critical hit is the sum of the critical
   * hit probabilities for 1, 2 and 3 hits with weights 1,2 and 3.
   */
  @Test
  ...
  /**
   * When two or more items are involved the calculations are a bit more complex. The damage dealt
   * by 1 damage has to take into account the number of hits from a critical hit. A 3-roll critical
   * strike has 3 chances to hit this item. The probability that the item will be hit is X~bin(n,p)
   * where n is the number of rolls and p is the chance that the item will be hit in the roll. The
   * binomial probability needs to be weighted with the number of hits. For example P(X=3) shall be
   * weighted by 3 to produce the correct damage amount.
   */
  @Test
  public void testTwoItems() {
    ...
    ans0 +=
        0.03
            * (3 * p_hit0 * (1 - p_hit0) * (1 - p_hit0) * 1
                + 3 * p_hit0 * p_hit0 * (1 - p_hit0) * 2
                + p_hit0 * p_hit0 * p_hit0 * 3);
    ans1 +=
        0.03
            * (3 * p_hit1 * (1 - p_hit1) * (1 - p_hit1) * 1
                + 3 * p_hit1 * p_hit1 * (1 - p_hit1) * 2
