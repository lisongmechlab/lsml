...
public class BurstDamageOverTimeTest {
  ...
  /**
   * {@link BurstDamageOverTime#calculate(double, double)} shall calculate the result correctly
   * taking range falloff and weapon cool downs into account.
   */
  @Test
  public final void testCalculate() throws Exception {
    ...
    items.add(erppc);
    final double time =
        erllas.getExpectedFiringPeriod(null) * 3 + erllas.getDuration(null) / 2; // 3.5 ER LLAS
    ...
    expected +=
        (int) (time / ac20.getExpectedFiringPeriod(null) + 1)
            * ac20.getDamagePerShot()
            * ac20.getRangeEffectiveness(500, null);
    expected +=
        (int) (time / erppc.getExpectedFiringPeriod(null) + 1)
            * erppc.getDamagePerShot()
            * erppc.getRangeEffectiveness(500, null);
    assertEquals(expected, burst, 1E-6);
  }

  /**
   * The implementation caches partial results. So even if we change parameters, the result shall be
   * calculated for the correct parameters.
   */
  @Test
  ...
  /**
   * Test that a hypothetical double fire capable ballistic weapon with a minimum range is correctly
   * calculated for automatic range.
   */
  @Test
  public final void testCalculate_MinRangeBallisticDoubleFire() {
    final BallisticWeapon weapon = mock(BallisticWeapon.class);
    final WeaponRangeProfile.Range optimal = new WeaponRangeProfile.Range(100.0, 200.0);
    ...
    when(weapon.getRangeEffectiveness(anyDouble(), any()))
        .thenAnswer(
            aInvocation -> {
              final double x = aInvocation.<Double>getArgument(0);
              if (x < optimal.minimum || x > optimal.maximum) {
                return 0.0;
              }
              return 1.0;
            });
  ...
  /**
   * {@link BurstDamageOverTime#calculate(double, double)} shall only calculate for the weapons in
