...
public class AlphaStrikeGraphModelTest {
  ...
  public void testIssue792() {
    final Weapon lrm15 = TestHelpers.makeWeapon(0, 180, 900, 900, true, 3.75, "LRM-15", modifiers);
    final Weapon lrm20 = TestHelpers.makeWeapon(0, 180, 900, 900, true, 4.65, "LRM-20", modifiers);

    weapons.add(lrm15);
    weapons.add(lrm20);

    when(alphaStrike.getWeaponDamageContribution(anyDouble()))
        .thenAnswer(
            i -> {
              final Map<Weapon, Double> ratio = new HashMap<>();
              ratio.put(lrm15, 15.0);
              ratio.put(lrm20, 20.0);
              return ratio;
            });
