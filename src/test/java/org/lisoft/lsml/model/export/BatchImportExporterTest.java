...
public class BatchImportExporterTest {
  ...
  private final String code1http =
      "http://t.li-soft.org/?l=rgATKDcFMhJNBzcFMCjne6%2FupzrMmOtmOq23MJbMdVtmOtk%3D";
  ...
  @Captor ArgumentCaptor<Throwable> errorArguments;
  ...
  public void testExport_EmptyDirectories() throws EncodingException {
    ...
    setupDir(rootName, root, singletonList(sub1), EMPTY_LIST);
    setupDir(sub1Name, sub1, singletonList(sub1sub1), EMPTY_LIST);
    setupDir(sub1sub1Name, sub1sub1, EMPTY_LIST, singletonList(loadout1));
  ...
  public void testExport_ManyLoadoutsHTTP() throws EncodingException {
    ...
    setupDir(dirName, root, EMPTY_LIST, asList(loadout1, loadout2, loadout3));
  ...
  public void testExport_NestedDirectories() throws EncodingException {
    ...
    setupDir(rootName, root, asList(sub1, sub2), singletonList(loadout1));
    setupDir(sub1Name, sub1, asList(sub1sub1, sub1sub2), singletonList(loadout2));
    setupDir(sub2Name, sub2, EMPTY_LIST, singletonList(loadout3));
    setupDir(sub1sub1Name, sub1sub1, EMPTY_LIST, asList(loadout4, loadout5));
    setupDir(sub1sub2Name, sub1sub2, EMPTY_LIST, singletonList(loadout6));
  ...
  public void testImport_BadFormatReportedSpec() throws Exception {
    ...
    String data = "[foobar]" + "\n";
    final String rubbish = "{rubbish name " + badLoadout;
    data += rubbish + "\n";

    cut.parse(data);

    verify(coder, never()).parse(badLoadout);
    verify(errorReporting)
        .error(eq("Parse error"), eq("Unable to parse line: " + rubbish), errorArguments.capture());
  ...
  public void testImport_BrokenLinkReportedSpec() throws Exception {
    final String badLoadout = "lsml://broken";
    String data = "[foobar]" + "\n";
    final String rubbishData = "{rubbish name }" + badLoadout;
    data += rubbishData + "\n";
  ...
  private void setupDir(
      String aName,
      GarageDirectory<Loadout> aDir,
      List<GarageDirectory<Loadout>> aChildren,
      List<Loadout> aLoadouts) {
    when(aDir.getDirectories()).thenReturn(unmodifiableList(aChildren));
