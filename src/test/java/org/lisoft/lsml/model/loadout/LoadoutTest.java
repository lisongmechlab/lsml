...
public abstract class LoadoutTest {
  ...
  public void testCanEquipDirectly_ComponentError() throws Exception {
    ...
    final EquipResult.EquipResultType[] resultTypes =
        new EquipResult.EquipResultType[] {
          EquipResult.EquipResultType.NoFreeHardPoints,
              EquipResult.EquipResultType.NoComponentSupport,
          EquipResult.EquipResultType.NotEnoughSlots,
              EquipResult.EquipResultType.ComponentAlreadyHasCase
        };
    ...
    for (final ConfiguredComponent component : components) {
      ...
      final EquipResult result =
          EquipResult.make(component.getInternalComponent().getLocation(), resultTypes[typeIndex]);
      when(component.canEquip(item)).thenReturn(result);
      typeIndex = (typeIndex + 1) % resultTypes.length;
    }
    assertEquals(
        EquipResult.make(EquipResultType.NoFreeHardPoints),
  ...
  public void testCanEquipDirectly_EngineHs() throws Exception {
    ...
    final HeatSink item =
        makeTestItem(0.0, 3, HardPointType.NONE, true, true, true, HeatSink.class);
    for (final ConfiguredComponent component : components) {
  ...
  public void testCanEquipDirectly_EnoughGlobalSlots() throws Exception {
    ...
    assertEquals(
        EquipResult.make(EquipResultType.Success), makeDefaultCUT().canEquipDirectly(item));
  }
  ...
  public void testCanEquipDirectly_NoEngineHs() throws Exception {
    ...
    assertEquals(
        EquipResult.make(EquipResultType.NotEnoughSlots), makeDefaultCUT().canEquipDirectly(item));
  }
  ...
  public void testCanEquipDirectly_TooFewSlots() throws Exception {
    ...
    chassisSlots = componentSlots * components.length;
    final Item item =
        makeTestItem(0.0, components.length + 1, HardPointType.NONE, true, true, false);
    for (final ConfiguredComponent component : components) {
      when(component.getSlotsUsed()).thenReturn(componentSlots - 1);
      when(component.getSlotsFree()).thenReturn(1);
      when(component.canEquip(item)).thenReturn(EquipResult.SUCCESS);
  ...
  public void testCanEquipDirectly_TooHeavy() throws Exception {
    final Item item =
        makeTestItem(
            ...
            true);
    assertEquals(
        EquipResult.make(EquipResultType.TooHeavy), makeDefaultCUT().canEquipDirectly(item));
  }

  @Test
  public void testCanEquipGlobal_AllowedAmountReached() {
    final ActiveProbe item =
        makeTestItem(0.0, 0, HardPointType.NONE, true, true, true, ActiveProbe.class);
    ...
    assertEquals(
        EquipResult.make(EquipResultType.TooManyOfThatType), makeDefaultCUT().canEquipGlobal(item));
  }
  ...
  public void testCanEquipGlobally_AllComponentsHaveCase() {
    ...
    assertEquals(
        EquipResult.make(EquipResultType.EverythingAlreadyHasCase),
  ...
  public final void testGetEquipmentModifiers() {
    ...
    final ActiveProbe item1 =
        mock(ActiveProbe.class); // Use ActiveProbe as it implements ModifierEquipment
  ...
  protected Item makeTestItem(
      double aMass,
      int aNumCriticals,
      HardPointType aHardPointType,
      boolean aIsCompatible,
      boolean aIsAllowed,
      boolean aIsAllowedOnAllComponents) {
    return makeTestItem(
        aMass,
        aNumCriticals,
        aHardPointType,
        aIsCompatible,
        aIsAllowed,
        aIsAllowedOnAllComponents,
        Item.class);
  }

  protected <T extends Item> T makeTestItem(
      double aMass,
      int aNumCriticals,
      HardPointType aHardPointType,
      boolean aIsCompatible,
      boolean aIsAllowed,
      boolean aCanEquipOnAllComponents,
      Class<T> aClass) {
