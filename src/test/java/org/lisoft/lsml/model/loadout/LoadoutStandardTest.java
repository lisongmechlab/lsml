...
public class LoadoutStandardTest extends LoadoutTest {
  ...
  public void testCanEquip_NoJJCapactity() {
    ...
    Assert.assertEquals(
        EquipResult.make(EquipResultType.JumpJetCapacityReached),
  ...
  public void testCanEquip_XLEngine11SlotsFree() {
    ...
    final Engine engine =
        makeTestItem(0.0, engineSlots, HardPointType.NONE, true, true, true, Engine.class);
    final Internal side =
        makeTestItem(0.0, sideSlots, HardPointType.NONE, true, true, true, Internal.class);
    ...
    when(components[Location.CenterTorso.ordinal()].canEquip(engine))
        .thenReturn(EquipResult.SUCCESS);

    Assert.assertEquals(
        EquipResult.make(EquipResultType.NotEnoughSlots),
        makeDefaultCUT().canEquipDirectly(engine));
  }
  ...
  public void testCanEquip_XLEngine12SlotsFree() {
    ...
    Assert.assertEquals(EquipResult.SUCCESS, makeDefaultCUT().canEquipDirectly(engine));
  }
  ...
  public void testCanEquip_XLEngineNoSpaceLeftTorso() {
    ...
    Assert.assertEquals(
        EquipResult.make(Location.LeftTorso, EquipResultType.NotEnoughSlotsForXLSide),
        makeDefaultCUT().canEquipDirectly(engine));
  }
  ...
  public void testCanEquip_XLEngineNoSpaceRightTorso() {
    ...
    Assert.assertEquals(
        EquipResult.make(Location.RightTorso, EquipResultType.NotEnoughSlotsForXLSide),
        makeDefaultCUT().canEquipDirectly(engine));
  }
  ...
  public final void testGetHeatSinksCount() {
    ...
    Assert.assertEquals(8, makeDefaultCUT().getTotalHeatSinksCount());
    Assert.assertEquals(5, makeDefaultCUT().getExternalHeatSinksCount());
  }
  ...
  public void testGetSlotsUsed() {
    ...
    Assert.assertEquals(expectedSlots, makeDefaultCUT().getSlotsUsed());
  }
  ...
  protected Loadout makeDefaultCUT() {
    ...
    return new LoadoutStandard(
        (ConfiguredComponentStandard[]) components,
        (ChassisStandard) chassis,
        upgradesMutable,
        weaponGroups);
  }
}
