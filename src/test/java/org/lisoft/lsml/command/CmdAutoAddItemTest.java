...
public class CmdAutoAddItemTest {
  ...
  /**
   * {@link CmdAutoAddItem} shall add an item to the first applicable slot in this loadout. Order
   * the items are added is: RA, RT, RL, HD, CT, LT, LL, LA
   */
  @Test
  ...
  public void testAddItem_engineHS() throws Exception {
    ...
    verify(xBar, times(2))
        .post(new ItemMessage(loadout.getComponent(Location.CenterTorso), Type.Added, shs, -1));
    verify(xBar).post(new ItemMessage(loadout.getComponent(Location.RightArm), Type.Added, shs, 0));
    assertTrue(
        loadout.getComponent(Location.CenterTorso).getItemsEquipped().contains(shs)); // 1 remaining
  ...
  public void testApply_XLEnginePerformance() throws Exception {
    ...
    // Execute
    stack.pushAndApply(
        new CmdAutoAddItem(loadout, xBar, ItemDB.lookup("XL ENGINE 200"), loadoutFactory));
  }

  /**
   * {@link CmdAutoAddItem} shall try to move items in order to make room for the added item if
   * there is no room in any component with a hard point but there are items that could be moved to
   * make room.
   */
  @Test
  public void testMoveItem() throws Exception {
    ...
    stack.pushAndApply(new CmdSetHeatSinkType(xBar, loadout, UpgradeDB.IS_DHS));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.RightTorso), ItemDB.DHS));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.RightTorso), ItemDB.DHS));
  ...
  public void testMoveItem_() throws Exception {
    ...
    final LoadoutStandard loadout =
        (LoadoutStandard) loadoutFactory.produceEmpty(ChassisDB.lookup("CTF-IM"));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.RightTorso), ac10));
    ...
    assertTrue(
        loadout.getComponent(Location.RightArm).getItemsEquipped().contains(ac10)
  ...
  public void testMoveItem_Bug1() throws Exception {
    // Setup
    final LoadoutStandard loadout =
        (LoadoutStandard) loadoutFactory.produceEmpty(ChassisDB.lookup("BNC-3M"));
    stack.pushAndApply(new CmdSetHeatSinkType(xBar, loadout, UpgradeDB.IS_DHS));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.RightArm), ItemDB.DHS));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.RightArm), ItemDB.DHS));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.RightTorso), ItemDB.DHS));
    ...
    stack.pushAndApply(
        new CmdAddItem(
            xBar,
            loadout,
            loadout.getComponent(Location.RightTorso),
    ...
    stack.pushAndApply(
        new CmdAddItem(
            xBar, loadout, loadout.getComponent(Location.Head), ItemDB.lookup("MEDIUM LASER")));
    stack.pushAndApply(
        new CmdAddItem(
            xBar,
            loadout,
    ...
    stack.pushAndApply(
        new CmdAddItem(
            xBar,
            loadout,
            loadout.getComponent(Location.LeftTorso),
            ItemDB.lookup("MEDIUM LASER")));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.LeftTorso), ItemDB.DHS));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.LeftTorso), ItemDB.DHS));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.LeftTorso), ItemDB.DHS));
    stack.pushAndApply(
        new CmdAddItem(xBar, loadout, loadout.getComponent(Location.LeftArm), ItemDB.DHS));
    // There is one free hard point in CT but no free slots, LRM10 must be swapped
    // with LRM 5
  ...
  /**
   * {@link CmdAutoAddItem} shall throw an {@link EquipResult} if the item cannot be auto added on
  ...
  /**
   * {@link CmdAutoAddItem} shall be able to swap items in addition to just moving one at a time.
   * Otherwise there we miss some solutions.
   */
  @Test
  public void testMoveItem_SwapItems() throws Exception {
    ...
    assertTrue(
        ListArrayUtils.equalsUnordered(ct.getItemsEquipped(), Arrays.asList(engine, lrm5, lrm5)));
