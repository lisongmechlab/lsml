...
public class CmdSetArmourTest {
  ...
  @Mock private MessageXBar messageRecipient;
  ...
  public CmdSetArmour makeCUT(int aSetArmour, boolean aSetIsManual) {
    ...
    Mockito.when(mlc.armourUpgrade.getArmourMass(anyInt()))
        .thenAnswer(
            aInvocation -> {
    ...
    return new CmdSetArmour(
        messageRecipient, mlc.loadout, mlc.ct, armourSide, aSetArmour, aSetIsManual);
  }
  ...
  /**
   * Setting the armour automatically after a manual set produce correct result and override manual
   * set flag.
   */
  @Test
  ...
  /**
   * Apply should correctly handle numerical precision problems that arise from armour amounts that
   * result in irrational tonnages.
   */
  @Test
  ...
  /**
   * Apply shall successfully change the armour value if called with an armour amount less than the
   * current amount and the 'mech is over-tonnage.
   */
  @Test
  ...
  /**
   * Attempting to add armour that would cause the mass limit on the loadout to be exceeded shall
   * result in an {@link EquipResult} when the operation is applied.
   *
   * <p>It must not be thrown on creation as there may be a composite operation that will be
   * executed before this one that reduces the armour so that this operation will succeed.
   */
  ...
  /**
   * Attempting to set armour that is more than the side can support (but less than free tonnage)
   * shall fail with an {@link EquipResult}.
   */
  @Test(expected = EquipException.class)
  ...
  /**
   * Two set armour operations can coalesce if they refer to the same (equality is not enough)
  ...
  public final void testCanCoalescele() {
    ...
    final CmdSetArmour cut1 =
        new CmdSetArmour(messageRecipient, mlc.loadout, part1, ArmourSide.FRONT, newArmour, true);
    final CmdSetArmour cut2 =
        new CmdSetArmour(messageRecipient, mlc.loadout, part1, ArmourSide.FRONT, newArmour, false);
    final CmdSetArmour cut3 =
        new CmdSetArmour(messageRecipient, mlc.loadout, part1, ArmourSide.BACK, newArmour, true);
    final CmdSetArmour cut4 =
        new CmdSetArmour(messageRecipient, mlc.loadout, part2, ArmourSide.FRONT, newArmour, true);
    final CmdSetArmour cut5 =
        new CmdSetArmour(
            messageRecipient, mlc.loadout, part1, ArmourSide.FRONT, newArmour - 1, true);
  ...
  /**
   * Any attempt to create an {@link CmdSetArmour} with negative armour shall throw an {@link
  ...
  /**
   * Any attempt to create a {@link CmdSetArmour} with more armour than the internal component can
   * hold (100% failure regardless of current armour value) shall throw an {@link
   * IllegalArgumentException} on construction.
   */
  @Test(expected = IllegalArgumentException.class)
  ...
  private void applyUndoTestTemplate(
    ...
    if (messageRecipient != null) {
      inOrder
          .verify(messageRecipient)
          .post(new ArmourMessage(mlc.ct, Type.ARMOUR_CHANGED, aManualSet));
    }
    inOrder.verify(mlc.ct).setArmour(armourSide, armour, aWasManual);
    if (messageRecipient != null) {
      inOrder
          .verify(messageRecipient)
          .post(new ArmourMessage(mlc.ct, Type.ARMOUR_CHANGED, aWasManual));
    }
  }
}
