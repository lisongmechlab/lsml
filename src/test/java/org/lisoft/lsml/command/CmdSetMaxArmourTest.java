...
public class CmdSetMaxArmourTest {
  ...
  @Mock private MessageDelivery xBar;
  ...
  public CmdSetMaxArmour makeCut(double aRatio, boolean aManual) {
    ...
    for (final Location loc : Location.values()) {
      ...
      Mockito.when(mlc.loadout.getComponent(loc).getArmourMax(any(ArmourSide.class)))
          .thenReturn(armourMax);
      Mockito.when(mlc.loadout.getComponent(loc).getInternalComponent().getArmourMax())
          .thenReturn(armourMax);
    }
  ...
  public void testApply() throws Exception {
    ...
    for (final ConfiguredComponent component : components) {
      ...
      if (loc.isTwoSided()) {
        ...
        inOrder
            .verify(component)
            .setArmour(eq(ArmourSide.FRONT), frontCaptor.capture(), eq(manual));
