...
public class CmdToggleItemTest {
  ...
  @Before
  public void setup() {
    Location location = Location.CenterTorso;

    ComponentOmniMech internalComponent = mock(ComponentOmniMech.class);
  ...
  public final void testDescribe() {
    ...
    final CmdToggleItem cmdToggleItem =
        new CmdToggleItem(msgDelivery, loadout, component, ItemDB.LAA, true);
  ...
  public final void testToggle_NotAllowed() throws Exception {
    ...
    when(component.getToggleState(ItemDB.LAA)).thenReturn(true);
    when(component.getToggleState(ItemDB.HA)).thenReturn(false);
    when(component.canToggleOn(ItemDB.HA))
        .thenReturn(EquipResult.make(EquipResultType.NoComponentSupport));
