...
public class CmdGarageAddDirectoryTest {
  ...
  public void testApplyDestinationIsLeaf() throws Exception {
    try {
      ...
      new CmdGarageAddDirectory<>(
              delivery, new GaragePath<>(fromPath("/2/d/", tgt.root), newObject), newDir)
          .apply();
  ...
  public void testApplyExistsWithValue() throws Exception {
    try {
      new CmdGarageAddDirectory<>(delivery, fromPath("/", tgt.root), new GarageDirectory<>("x"))
          .apply();
      fail("Expected exception!");
    } catch (final GarageException e) {
      assertTrue(e.getMessage().toLowerCase().contains("exists"));
  ...
  public void testApplyUndo() throws Exception {
    ...
    final CmdGarageAddDirectory<NamedObject> cut =
        new CmdGarageAddDirectory<>(delivery, dstDirPath, newDir);
