...
public class CmdGarageMoveTest {
  ...
  public void testDescribe() throws IOException {
    final CmdGarageMove<NamedObject> cut =
        new CmdGarageMove<>(md, fromPath("/2", tgt.root), fromPath("/x", tgt.root));
  ...
  public void testMoveDirectory() throws Exception {
    ...
    verify(md)
        .post(new GarageMessage<>(GarageMessageType.ADDED, new GaragePath<>(dstPath, tgt.dird)));
    ...
    cut.undo();
    verify(md)
        .post(new GarageMessage<>(GarageMessageType.REMOVED, new GaragePath<>(dstPath, tgt.dird)));
  ...
  public void testMoveObject() throws Exception {
    ...
    cut.undo();
    verify(md)
        .post(new GarageMessage<>(GarageMessageType.REMOVED, new GaragePath<>(dstPath, tgt.z)));
    verify(md).post(new GarageMessage<>(GarageMessageType.ADDED, srcPath));
    tgt.assertUnmodified();
  }
