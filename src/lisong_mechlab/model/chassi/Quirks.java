...
public class Quirks implements MovementModifier, WeaponModifier, HealthModifier, HeatModifier{
   ...
   static{
      ...
      for(Location location : Location.values()){
         KNOWN_QUIRKS.put("internal_resist_" + location.shortName().toLowerCase() + "_multiplier", QuirkBenefit.POSITIVE_GOOD);
         KNOWN_QUIRKS.put("internal_resist_" + location.shortName().toLowerCase() + "_additive", QuirkBenefit.POSITIVE_GOOD);
         KNOWN_QUIRKS.put("armor_resist_" + location.shortName().toLowerCase() + "_multiplier", QuirkBenefit.POSITIVE_GOOD);
         KNOWN_QUIRKS.put("armor_resist_" + location.shortName().toLowerCase() + "_additive", QuirkBenefit.POSITIVE_GOOD);
      }
   ...
   private QuirkBenefit isPositiveGood(Quirk aQuirk){
      ...
      return QuirkBenefit.POSITIVE_GOOD; // Most quirks are positive good, assume this if we don't know about the quirk.
   }
   ...
   public boolean affectsWeapon(Weapon aWeapon){
      if( aWeapon instanceof MissileWeapon && (quirks.containsKey("missile_cooldown_multiplier") || quirks.containsKey("missile_heat_multiplier")) )
         return true;
      if( aWeapon instanceof EnergyWeapon && (quirks.containsKey("energy_cooldown_multiplier") || quirks.containsKey("energy_heat_multiplier")) )
         return true;
      if( aWeapon instanceof BallisticWeapon
          && (quirks.containsKey("ballistic_cooldown_multiplier") || quirks.containsKey("ballistic_heat_multiplier")) )
         return true;
      return false;
   }
