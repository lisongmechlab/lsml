...
public class LoadoutPartTest{
   ...
   public void testCanAddItem_jumpJets(){
      for(Part p : Part.values()){
         ...
         if( p == Part.RightLeg || p == Part.LeftLeg || p == Part.RightTorso || p == Part.LeftTorso || p == Part.CenterTorso )
            assertTrue(cut.canAddItem(jumpjet));
         else
   ...
   public void testCanAddItem_jumpJets_Failures(int globalSlots, int localSlots, double freeMass, int jjCapacity, int jjOccupacy){
      for(Part p : Part.values()){
         // Setup
         Item jumpjet = ItemDB.lookup("JUMP JETS - CLASS V");
         Chassi chassi = Mockito.mock(Chassi.class);
   ...
   public void testCanAddItem_CASE(Part aPart) throws Exception{
      ...
      if( aPart == Part.LeftTorso || aPart == Part.RightTorso ){
         assertTrue(cut.canAddItem(ItemDB.lookup("C.A.S.E.")));
      }
      else{
         assertFalse(cut.canAddItem(ItemDB.lookup("C.A.S.E.")));
      }
   }
   ...
   public void testCanAddEngine(Part aPart, int ctFreeSlots, int globalSlots, double freeMass, int engineMin, int engineMax){
      ...
      if( aPart == Part.CenterTorso ){
         assertTrue(cut.canAddItem(ItemDB.lookup("STD ENGINE 300")));
      }
      else{
         assertFalse(cut.canAddItem(ItemDB.lookup("STD ENGINE 300")));
      }
   }
   ...
   public void testCanAddItem_xlEngineFailures(int ltFreeSlots, int rtFreeSlots, int ctFreeSlots, int globalSlots, double freeMass,
                                               Integer engineMin, Integer engineMax) throws Exception{
      // Setup
      ...
      // Execute & Verify
      assertFalse(cut.canAddItem(ItemDB.lookup("XL ENGINE 300")));
   }
   ...
   public void testCanAddItem_stdEngineFailures(int ctFreeSlots, int globalSlots, double freeMass, Integer engineMin, Integer engineMax)
                                                                                                                                        throws Exception{
      // Setup
      Chassi chassi = Mockito.mock(Chassi.class);
      Mockito.when(chassi.getEngineMax()).thenReturn(engineMax);
    ...
    */
   @Test
   public void testCanAddItem_internal(){
      Loadout loadout = Mockito.mock(Loadout.class);
      Mockito.when(loadout.getFreeMass()).thenReturn(100.0);
    ...
    */
   @Test
   public void testCanAddItem_TooFewSlots(){
      // Setup
      Loadout loadout = Mockito.mock(Loadout.class);
      Mockito.when(loadout.getFreeMass()).thenReturn(100.0);
    ...
    */
   @Test
   public void testCanAddItem_Artemis(){
